...
import React, { useState, useEffect } from 'react';
import { FaCalendarAlt, FaStar, FaEye, FaTimes } from "react-icons/fa";
import QRCode from "react-qr-code";
import logo from "../assets/hangout.png";
import { useParams } from "react-router-dom";
import axios from 'axios';
...
import { useNavigate } from 'react-router-dom';
import { Form, Input, Rate, Select, InputNumber, Spin, message } from 'antd';
import { Button } from "@mui/material";
import { ToastContainer, toast } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";
import Avatar from "../assets/images (7).png"; 
import { Share, PhoneOutlined, MailOutlined } from "@mui/icons-material";
const apiKey2 = import.meta.env.VITE_API_KEY

// Booking Modal Component
const BookingModal = ({ visible, onClose, event, onBookingSuccess }) => {
  const [form] = Form.useForm();
  ...
  const ticketPrices = {
    standard: event?.price || 0,
    vip: event?.vip_price || 0,
    early_bird: event?.early_bird_price || 0
  };

  const calculateTotal = () => {
    return (ticketPrices[ticketType] || 0) * quantity;
  };
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      const response = await axios.post(
        `${API_ROUTE}/event/${event.slug}/book/`,
        bookingData,
        { headers: { Authorization: `Token ${token}` } }
      );

      if (response.status === 201) {
        message.success('Booking confirmed! Check your email for ticket details.');
    ...
    <AnimatePresence>
      {visible && (
        ...
        >
          ...
          >
            <div className="sticky top-0 bg-orange-500 text-white p-4 rounded-t-2xl flex justify-between items-center">
              <div>
                <h2 className="text-xl font-bold">Reserve Your Spot</h2>
                <p className="text-sm opacity-90">{event?.event_title}</p>
              </div>
            ...
            <div className="p-6">
              <Form form={form} layout="vertical" onFinish={handleSubmit}>
                ...
                <div className="mb-4">
                  <label className="block font-semibold mb-2">Select Ticket Type</label>
                  <div className="grid grid-cols-3 gap-2">
                      ...
                      <motion.div 
                        ...
                        className={`p-3 rounded-lg cursor-pointer transition-all text-center ${
                          ticketType === 'standard' 
                            ...
                            : 'bg-gray-100 border-2 border-gray-200 hover:border-orange-300'
                        }`}
                    ...
                    {event?.vip_price > 0 && (
                      ...
                      >
                        ...
                        <div className="text-sm">₦{event.vip_price?.toLocaleString()}</div>
                      </motion.div>
                    )}
                    {event?.early_bird_price > 0 && (
                      <motion.div 
                        ...
                        onClick={() => handleTicketTypeChange('early_bird')}
                      >
                        <div className="font-bold">Early Bird</div>
                        <div className="text-sm">₦{event.early_bird_price?.toLocaleString()}</div>
                      </motion.div>
                    )}
                  </div>
                </div>
                ...
                <div className="bg-orange-50 p-4 rounded-lg mb-4">
                  <div className="flex justify-between mb-2">
                    ...
                    <span>₦{ticketPrices[ticketType]?.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between mb-2">
                    <span>Quantity:</span>
                ...
                <Form.Item label="Special Requests (Optional)" name="special_requests">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-orange-500 text-white py-3 rounded-lg font-semibold hover:bg-orange-600 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                >
                  {loading ? <Spin /> : 'Confirm Booking'}
                </button>

                <p className="text-xs text-gray-500 text-center mt-4">
                  By confirming, you agree to our terms and conditions. A confirmation email will be sent to your email address.
                </p>
              </Form>
            </div>
          </motion.div>
...
export default function EventDetails() {
  ...
  useEffect(() => {
    const updateDate = () => {
      ...
      setFormattedDate(`${days > 0 ? `${days}d ` : ""}${hours}h ${minutes}m ${seconds}s`);
    };
  ...
  const handlePrev = () => {
    if (currentIndex === 0) {
      setCurrentIndex(totalEvents - eventsPerPage);
    } else {
      setCurrentIndex(currentIndex - eventsPerPage);
    }
  };
  ...
  const handleFinish = async (values) => {
    ...
    try {
      ...
      if (response.status === 201) {
        ...
        form.resetFields();
        // Refresh reviews
        const reviewsResponse = await axios.get(`${API_ROUTE}get-live-event-reviews/${slug}/`);
      ...
      console.error("Error submitting review:", error.response?.data || error.message);
      toast.error('An error occurred while submitting the review or cannot send review twice');
    }
  };
    ...
    <div className="bg-gray-50">
      ...
      <BookingModal
        ...
        onBookingSuccess={(booking) => {
          ...
          toast.success(`Booking confirmed! Your ticket code: ${booking.ticket_code}`);
        }}
      />
      ...
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="flex flex-col md:flex-row gap-6">
          <div className="relative w-full h-full rounded-lg overflow-hidden">
            {images?.length > 0 ? (
              <img src={images} alt='event' className="w-full h-full object-cover" />
            ) : videos ? (
              <video src={videos} className="w-full h-full object-cover" controls />
            ) : (
              <div className="w-full h-full flex items-center justify-center bg-gray-200">
                <p className="text-gray-500">No Media Available</p>
              </div>
          ...
          <div className="bg-white border border-gray-300 p-6 rounded-xl md:w-2/5">
            ...
            <div className="flex justify-between items-center mt-6">
              ...
              <div className="flex items-center">
                {Array.from({ length: 5 }, (_, index) => (
                  <FaStar key={index} className={`text-lg ${index < Math.floor(rating) ? "text-yellow-500" : "text-gray-300"}`} />
                ))}
                <span className="ml-2 text-lg font-semibold text-gray-800">{rating}/5</span>
              </div>
            </div>
            ...
            <motion.button 
              ...
              className="mt-6 bg-orange-500 text-white px-6 py-3 rounded-xl font-semibold text-lg hover:bg-orange-600 transition duration-300 w-full"
            >
              Reserve a Spot
            </motion.button>
          </div>
        ...
        <div>
          <button onClick={sharePlace} className="flex items-center px-4 mt-4 py-2 text-blue-500 hover:text-blue-700 bg-gray-100 hover:bg-gray-200 rounded-lg transition">
            <Share className="w-5 h-5 mr-2" /> Share and Invite
          </button>
        </div>
        ...
        {organizer_name && (
          <div className="mt-10">
            <h3 className="text-xl font-semibold">Organized by</h3>
            <div className="border border-gray-200 p-4 rounded mt-3 bg-white">
              <p className="text-gray-600">{organizer_name}</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="bg-orange-700 p-10 flex flex-col md:flex-row justify-between items-center mt-10">
        <div className="max-w-lg text-center md:text-left">
          ...
          <p className="mt-2 text-lg text-white">Find your next plan on Essential NG</p>
        </div>
        <div className="flex flex-col items-center bg-white p-6 md:p-10 rounded-xl shadow-lg mt-6 md:mt-0">
          <QRCode value="https://edirect.ng.com" size={200} />
          <p className="mt-4 text-xl font-semibold text-gray-900 text-center">Scan to download other e-Apps</p>
        </div>
      </div>
      ...
      <div className="mt-10 p-20">
        ...
        {reviews.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {reviews.map((review) => (
              <div key={review.id} className="border-2 border-gray-300 p-6 rounded-2xl bg-white hover:shadow-xl transition-shadow duration-300">
                <div className="flex items-center mb-4">
                  <img src={Avatar} alt="Profile" className="w-12 h-12 rounded-full border-2 border-gray-300 shadow-sm mr-4" />
                  <div>
                    <p className="text-lg font-semibold text-gray-900">{review.user_name}</p>
                    <p className="text-sm text-gray-500">{new Date(review.created_at).toLocaleDateString()}</p>
                  </div>
          ...
          <p className="text-gray-600 text-center text-lg font-medium">No reviews yet. Be the first to review.</p>
        )}
          ...
          {[5, 4, 3, 2, 1].map((star) => {
            ...
            return (
              <div key={star} className="flex items-center mt-2">
                <span className="w-12 text-gray-800">{star} star</span>
                <div className="w-full h-3 bg-gray-300 rounded-md ml-2">
                  <div className="h-3 bg-orange-500 rounded-md" style={{ width: `${percentage}%` }}></div>
                </div>
                <span className="w-10 text-right text-gray-700">{Math.round(percentage)}%</span>
              </div>
            );
      ...
      <div className="max-w-lg mt-8 ml-20 p-8 bg-white rounded-2xl border border-gray-300">
        <h2 className="text-2xl font-bold mb-4 text-gray-900 flex items-center">
          <span className="text-orange-500 text-3xl mr-2">★</span> Rate & Review
        </h2>
        <p className="text-gray-600 mb-6">Your feedback helps others make informed decisions. Share your experience!</p>

        <Form form={form} layout="vertical" onFinish={handleFinish} className="bg-gray-100 p-6 rounded-xl shadow-md">
          <Form.Item name="rating" label={<span className="font-medium text-gray-800">Your Rating</span>} rules={[{ required: true, message: 'Please provide a rating' }]}>
            <Rate className="text-yellow-500 text-2xl" />
          </Form.Item>
          <Form.Item name="comment" label={<span className="font-medium text-gray-800">Your Review</span>} rules={[{ required: true, message: 'Please provide a comment' }]}>
            <Input.TextArea rows={4} placeholder="Write your honest review here..." className="p-3 rounded-lg border border-gray-300 focus:border-orange-500 focus:ring-2 focus:ring-orange-200 transition shadow-sm bg-white" />
          </Form.Item>
          <Form.Item>
            <Button type="primary" htmlType="submit" block className="bg-orange-500 hover:bg-orange-600 text-white font-semibold py-3 rounded-lg transition duration-300 shadow-md">
              Submit Review
            </Button>
          </Form.Item>
        </Form>
      ...
      {/* Other Events You May Like */}
      <div className="mt-20 w-full px-6">
        <h1 className="text-4xl font-extrabold text-gray-900 text-center mb-8">Other Events You May Like</h1>
        <div className="relative">
          <div className="flex gap-6 justify-center items-center mb-20 flex-wrap">
            {currentEvents.map((event, index) => (
              <motion.div
                key={index}
                whileHover={{ scale: 1.05 }}
              ...
              >
                <div className="relative w-full h-56 rounded-t-xl overflow-hidden">
                  {event.image?.length > 0 ? (
                    <img src={event.image} alt={event.event_title} className="w-full h-full object-cover" />
                  ) : event.video ? (
                    <video src={event.video} className="w-full h-full object-cover" controls />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center bg-gray-300">
                      <p className="text-gray-500 text-sm">No Media Available</p>
                    </div>
                ...
                <div className="p-6">
                  ...
                  <button onClick={() => navigate(`/LiveEventDetails2/${event.slug}`)} className="mt-6 w-full border border-orange-600 text-orange-600 hover:bg-orange-600 hover:text-white px-6 py-3 rounded-md font-semibold transition-all hover:opacity-90">
                    Check Out Event
                  </button>
                </div>
              </motion.div>
          ...
          {/* Navigation Buttons */}
          <div className="absolute top-1/2 left-0 transform -translate-y-1/2 pl-4">
            <button onClick={handlePrev} className="bg-gray-800 text-white p-2 rounded-full shadow-md hover:bg-gray-600 transition-all">&#8592; Prev</button>
          </div>
          <div className="absolute top-1/2 right-0 transform -translate-y-1/2 pr-4">
            <button onClick={handleNext} className="bg-gray-800 text-white p-2 rounded-full shadow-md hover:bg-gray-600 transition-all">Next &#8594;</button>
          </div>
        </div>
      </div>
    </div>
